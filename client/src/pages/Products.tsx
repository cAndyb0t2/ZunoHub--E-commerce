...
export default function Products() {
  ...
  return <main className="container page">
    <div className="page-heading catalogue-heading"><div><span className="eyebrow">THE DMART AISLES</span><h1>{title}</h1><p>{products.length ? `${products.length} everyday essentials to choose from.` : "Find the everyday essentials your home needs."}</p></div><form className="catalogue-search" onSubmit={submitSearch}><Search size={17} aria-hidden="true" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search the store" aria-label="Search the store" /><button type="submit">Go</button></form></div>
